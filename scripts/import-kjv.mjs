...
function parseVerses(raw) {
  ...
  for (const line of text.split('\n')) {
    ...
    if (headingBook) {
      currentBook = headingBook;
      continue;
    }
    const match = trimmed.match(/^(\d+):(\d+)\s+(.+)$/);
    if (!match || !currentBook) continue;
    const chapter = Number(match[1]);
    const verse = Number(match[2]);
    verses.push({ book: currentBook.osisId, chapter, verse, text: match[3] });
  }
  return { text, verses };
}
...
function validate(verses, manifest, checksum) {
  ...
  if (manifest.source.expectedSha256 && manifest.source.expectedSha256 !== checksum) errors.push(`Source checksum ${checksum} does not match manifest ${manifest.source.expectedSha256}`);
  return { ok: errors.length === 0, errors, totals: { books: byBook.size, chapters: [...byBook.values()].reduce((sum, b) => sum + b.chapters.size, 0), verses: verses.length } };
}

function buildSql(manifest, checksum, verses) {
  const t = manifest.translation, s = manifest.source;
  const lines = ['begin;', `insert into public.translations (slug, abbreviation, name, language_code, versification, copyright_notice, rights_statement, is_public_domain, is_enabled, metadata) values (${sql(t.slug)}, ${sql(t.abbreviation)}, ${sql(t.name)}, ${sql(t.languageCode)}, ${sql(t.versification)}, ${sql('Public domain in the USA')}, ${sql(s.license)}, true, false, ${sql(JSON.stringify({ sourceManifest: 'data/kjv/source-manifest.json' }))}::jsonb) on conflict (slug) do update set metadata = excluded.metadata;`];
  for (const b of BOOKS) lines.push(`insert into public.books (osis_id, slug, name, common_abbreviation, testament, book_order, chapters_count) values (${sql(b.osisId)}, ${sql(b.slug)}, ${sql(b.name)}, ${sql(b.abbreviation)}, ${sql(b.testament)}, ${b.order}, ${b.chaptersCount}) on conflict (osis_id) do nothing;`);
  lines.push(`insert into public.data_sources (translation_id, source_key, source_type, title, url, license, attribution, checksum_sha256, imported_at, metadata) select id, ${sql(s.key)}, ${sql(s.type)}, ${sql(s.title)}, ${sql(s.url)}, ${sql(s.license)}, ${sql(s.attribution)}, ${sql(checksum)}, now(), ${sql(JSON.stringify({ catalogUrl: s.catalogUrl, accessedOn: s.accessedOn }))}::jsonb from public.translations where slug = ${sql(t.slug)} on conflict (source_key) do update set checksum_sha256 = excluded.checksum_sha256, metadata = excluded.metadata;`);
...
if (!validation.ok) {
  console.error(`KJV import validation failed. See ${REPORT_PATH}`);
